            ...
            <div class="row">
                <div class="col-12">
                    @if($order->return_order == 1)
                        <button type="button" class="btn btn-primary btn-block" title="Approve Return" onclick="approve({{ $order->id }})">Approve Return Request</button>
                        <form id="delete_form_{{ $order->id }}" method="get" action="{{ route('admin.return.orders.approve', $order->id) }}">
                        </form>
                    @elseif($order->return_order == 2)
                        <a class="btn btn-success btn-block disabled text-light">This Order is Returned</a>
                    @else
